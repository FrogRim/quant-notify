# Naver Cloud SMS OTP 검증 매뉴얼

이 문서는 **전화번호 OTP 로그인**을 Naver Cloud SMS로 검증할 때 쓰는 **보조 런북**입니다.
...
- `infra/.env.production`의 다음 키가 **프로덕션/테스트 발송**에 맞게 채워짐:
  ...
  - `NAVER_SMS_FROM` — **Naver 콘솔에 등록된 발신번호**와 동일해야 함
...
## 1. Naver Cloud 콘솔 확인 (테스트 전)

1. [Naver Cloud Platform](https://www.ncloud.com/) 로그인 → 본인이 쓰는 **프로젝트** 선택.
2. **Services → Simple & Easy Notification Service (SENS)** 또는 현재 쓰는 SMS 상품 메뉴로 이동 (콘솔 UI는 변경될 수 있음).
3. 아래를 확인합니다.
   - **SMS 서비스**가 생성되어 있고, 앱에 넣은 `NAVER_SMS_SERVICE_ID`와 동일한지.
   - **발신번호**가 등록·승인되어 있고, `NAVER_SMS_FROM`과 **숫자만 형식까지** 일치하는지 (하이픈 유무는 구현에 따라 다를 수 있으니 Naver 측 표기와 맞출 것).
   - **발송 한도·잔액**이 남아 있는지.

서비스 ID 형식은 환경에 따라 `ncp:sms:kr:...` 류 문자열인 경우가 많습니다. `infra/.env.production.example`과 동일한 패턴인지 비교합니다.
...
1. **API 컨테이너 로그** (VPS 저장소 루트에서):
   ...
   프로덕션에서 Naver 미설정 시 코드는 콘솔 대신 에러를 낼 수 있습니다. `auth.otp` 관련 메시지·스택을 확인합니다.

2. **환경 변수** 오타·공백, 특히 `NAVER_SMS_SERVICE_ID` / `NAVER_SMS_FROM`.
3. Naver 콘솔에서 **발송 실패 로그**가 있는지.
...
| 증상 | 우선 확인 |
|------|-----------|
| start 5xx / OTP 미수신 | Naver 키·발신번호·쿼타, `api` 로그 |
